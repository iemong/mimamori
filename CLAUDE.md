...
projects/             # プロジェクト設定 + knowledge + activity + notes + guard-logs
templates/
...
プロジェクトの `features.notes` が `true` の場合に有効。
`:memo:` プレフィックス付きの投稿を1日1ファイル（`projects/{slug}/notes/YYYY-MM-DD.md`）に蓄積します。

### 応答フォーマット

`:memo:` 投稿を受けたら、内容を判断して以下の形式で返してください:

- タスク（やること・調べること）の場合: `TASK: {内容}`
- メモ（情報・知識・記録）の場合: `MEMO: {内容}`

例:
- `:memo: CORSエラー調べる` → `TASK: CORSエラー調べる`
- `:memo: OAuth2はcode flowを使う` → `MEMO: OAuth2はcode flowを使う`

### タスク完了

メンションで「〇〇の件終わった」と言われたら、該当するタスクを完了にしてください。
エージェントはnotesファイルをReadで確認し、completeTaskで更新できます。

### 未完了タスク一覧

「今日のタスク」と聞かれたら、その日の未完了タスク（`- [ ]`）を返してください。
